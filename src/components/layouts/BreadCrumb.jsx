...
const DynamicUserBreadcrumb = ({ match }) => (
  <span>{categoryNameById[match.params.id]}</span>
);
        ...
        <ol className="inline-flex items-center space-x-1">
          {breadcrumbs.map(({ breadcrumb, match }, index) => (
            <li key={index} className="inline-flex items-center">
              <NavLink
                to={match.pathname}
                className={`inline-flex items-center font-medium text-sm mr-2 ${match.pathname === current ? 'text-neutral-500 hover:text-[#5C59E8]' : 'text-[#5C59E8] hover:text-neutral-500 '}  `}
              >
                {breadcrumb}
              </NavLink>
              {match.pathname === current ? (
